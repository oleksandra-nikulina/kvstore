...
/// Hash values and set/list members are modeled as `Bytes` (binary-safe,
/// like real Redis); hash *field names* are `String` instead, for the
/// same reason top-level keys are — see stage 4's `key_from_bytes`.
pub enum Value {
    Bytes(Bytes),
    List(VecDeque<Bytes>),
