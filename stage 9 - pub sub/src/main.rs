use kvstore_stage9::persistence::{Aof, replay};
...
async fn main() {
    ...
    let port: u16 = args
        ...
        .parse()
        .unwrap_or_else(|_| {
            eprintln!("usage: kvstore-stage9 [port] [aof_path]");
            process::exit(1);
        });
    ...
    println!("stage 9 KV store (with Pub/Sub) listening on {addr}, logging to {aof_path}");
